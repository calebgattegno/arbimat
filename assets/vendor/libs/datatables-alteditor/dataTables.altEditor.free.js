    ...
    module.exports = function (root, $)
    {
      ...
      if (!$ || !$.fn.dataTable)
      {
        //$ = require('datatables.net')(root, $).$;
				throw new Error('commented because of parcel');
      }
    ...
    $.extend(
      altEditor.prototype,
      {
        ...
        _initLanguage: function ()
        {
          ...
          this.language.duplicate = this.language.duplicate || {};
          this.language.duplicate = {
            title: this.language.duplicate.title || 'Duplicate record',
            button: this.language.duplicate.button || 'Duplicate'
          };
         ...
         * @private
         */
        _openEditModal: function (isDuplication = false)
        {
          ...
          var data = this.createDialog(
            columnDefs,
            isDuplication ? this.language.duplicate.title : this.language.edit.title,
            isDuplication ? this.language.duplicate.button : this.language.edit.button,
            this.language.modalClose, 'editRowBtn', 'altEditor-form',
            isDuplication ? 'duplicate' : 'edit'
          );
          ...
          for (var j in columnDefs)
          {
            if (columnDefs[j].name != null)
            {
              ...
              //DEVEX fix (added !== undefined)
              if (typeof selectedValue !== 'object' && selectedValue !== null && selectedValue !== undefined)
              {
              ...
              {
                if (this._quoteattr(selectedValue) === "true" || this._quoteattr(selectedValue) == "1")
              ...
              else if (columnDefs[j].type.indexOf("date") >= 0)
              {
                if (columnDefs[j].dateFormat !== "")
                {
                  ...
                  if (mDate && mDate.isValid())
                  {
                    ...
                    //devex: do not re-change value if already changed in date handler
                    continue;
                  }
                }
              }
         ...
         */
        _editRowData: function (isDuplication = false)
        {
          var that = this;
          var dt = this.s.dt;
          var adata = dt.rows({
          ...
          var $inputs = $('form[name="altEditor-form-' + this.random_id + '"]').find('select, textarea, input');
        ...
        _openDeleteModal: function ()
        {
          ...
          var formName = 'altEditor-form-' + this.random_id;
          var selector = this.modal_selector;
          var fill = function ()
          {
        ...
        _openAddModal: function ()
        {
          ...
          var data = this.createDialog(columnDefs, this.language.add.title, this.language.add.button,
            this.language.modalClose, 'addRowBtn', 'altEditor-form',
            'new'
            );
        ...
        completeColumnDefs: function ()
        {
          ...
          for (var i in dt.context[0].aoColumns)
          {
            ...
            columnDefs[i] = {
              ...
              defaultContent: (obj.defaultContent ? obj.defaultContent : undefined), // devex: setup default value when undefined
              allowedAdding: obj.allowedAdding ?? true, // devex: disable edit/add for specific fields
              allowedEditing: obj.allowedEditing ?? true, // devex: disable edit/add for specific fields
            }
          }
          return columnDefs;
        },
        ...
        */
        createDialog: function (columnDefs, modalTitle, buttonCaption, closeCaption, buttonClass, formName, type)
        {
          var isEdit = type == 'edit';
          var isDuplication = type == 'duplicate';
          ...
          {
            //is allowed
            var isAllowed = isEdit ? (columnDefs[j].allowedEditing ?? true) : (columnDefs[j].allowedAdding ?? true);
            var title = columnDefs[j].title.replace(/(<([^>]+)>)/gi, "").trim();

            //handle hidden fields (devex:)
            if (!isAllowed || columnDefs[j].type.indexOf("hidden") >= 0)
            {
              //do not allow editing existing ID if we're duplicating field
              if (columnDefs[j].name != 'id' || isDuplication == false)
                data += "<input type='hidden' "
                  + "id='" + this._quoteattr(columnDefs[j].name) + "' "
                  + fillAttrs(columnDefs[j], ['name', 'value'])
                  + (columnDefs[j].defaultContent ? ("value='" + columnDefs[j].defaultContent + "' ") : '') //devex: default value
                  + "></input>";
            }
            else
            {
              ...
              if (columnDefs[j].type.indexOf("select") >= 0)
              {
                ...
                data += "<select class='form-control" + (columnDefs[j].select2 ? ' select2' : '') + "' "
                  ...
                  + (columnDefs[j].defaultContent ? ("value='" + columnDefs[j].defaultContent + "' ") : '') //devex: otestovat
                  + ">" + options
                  + "</select>";
              }
              // Adding Text Area
          ...
          for (var j in columnDefs)
          {
            ...
            {
              // Require select2 plugin //DEVEX fix
